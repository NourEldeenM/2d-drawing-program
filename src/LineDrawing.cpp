...
void DrawParametricLine(HDC hdc, int x1, int y1, int x2, int y2, COLORREF c1, COLORREF c2){
    ...
    int alpha1 = x2 - x1, alpha2 = y2 - y1,alphar = r2 - r1, alphag = g2 - g1, alphab = b2 - b1;
    SetPixel(hdc, x1, y1, c1);
    double step = 1.0 / max(abs(alpha1), abs(alpha2));
    for (double t = 0; t <= 1; t += step)
        SetPixel(hdc, round(x1 + t * alpha1), round(y1 + t * alpha2), RGB(round(r1 + t * alphar), round(g1 + t * alphag), round(b1 + t * alphab)));
