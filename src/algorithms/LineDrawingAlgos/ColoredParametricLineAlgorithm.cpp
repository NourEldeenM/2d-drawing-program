...
class ColoredParametricLineAlgorithm : public DrawingAlgorithm
{
private:
...
public:
    ColoredParametricLineAlgorithm()
    {
        setName((char *)"Colored Parametric Line Algorithm");
    ...
    vector<pair<Point, Color>> draw(vector<Point> &inputPoints, vector<Color> drawingColors = {BLACK}) override
    {
        ...
        // Add the first point with start color
        Point firstPoint = {x1, y1};
        Color firstColor = {startR, startG, startB, 255};
        outputPoints.push_back({firstPoint, firstColor});
